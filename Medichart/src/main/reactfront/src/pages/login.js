...
function Login() {
  ...
  const kakaoURL = `https://kauth.kakao.com/oauth/authorize?client_id=${K_REST_API_KEY}&redirect_uri=${encodeURIComponent(
    K_REDIRECT_URI
  ...
  const NAVER_URL = `https://nid.naver.com/oauth2.0/authorize?client_id=${N_clientID}&response_type=code&redirect_uri=${encodeURIComponent(
    N_REDIRECT_URI
  )}&state=${stateString}`;
  ...
  const G_URL = `https://accounts.google.com/o/oauth2/v2/auth?response_type=code&client_id=${G_ClientID}&redirect_uri=${encodeURIComponent(
    G_REDIRECT_URI
